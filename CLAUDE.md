...
**Current state: live, 43 pages.** Homepage, all 8 category pages from the homepage cat-grid, the blog hub and 33 blog posts are built and shipped — not a scaffold. Contact channel is Max messenger, not phone — see "No phone calls" in PRODUCT.md Capabilities and Constraints before adding any contact CTA.
...
## Blog (33 posts live, ahead of the phased plan)
...
**All 18 posts expanded to 1500+ words (2026-09).** Initial drafts ran 392–686 words; each was expanded with genuine additional technical content (never padding) — real furniture/construction facts already established elsewhere in this doc (formaldehyde classes, kromka properties, hardware brands, standard dimensions) plus new adjacent topics per article (e.g. sink-cutout waterproofing, hinge/slide types, contract acceptance procedure). Two reusable components were added for the long-form length: `.article-toc` (an anchor-link table of contents right after the lead paragraph) and `.back-to-top` (fixed circular button, appears past 600px scroll, smooth-scrolls to `#top` on the header). Both are wired into `style.css`/`main.js` and present on all 18 posts — copy the same header `id="top"` + `<div class="article-toc">` + trailing `<a href="#top" class="back-to-top">` markup for any future long-form post. FAQ counts grew alongside the prose (most posts now have 9-10 Q&As, up from 5-6) — **whenever FAQ HTML items change, the `FAQPage` JSON-LD `mainEntity` array must be updated to match in the same edit**; a Python word-count/FAQ-parity script (see any recent commit for the exact regex) is the fast way to verify both counts before committing. Full Playwright pass confirmed after the expansion: zero horizontal overflow at 390/1024/1440px, all FAQ accordions and ToC anchor-jumps functional, back-to-top verified (smooth-scroll needs ~1s to settle at scrollY 0 — don't mistake mid-animation scrollY for a bug), zero broken internal links across all 28 site pages.

**15 more posts added (2026-09-24), written in batches of 3 against `on-page-seo.md`.** Topics: шкаф-купе vs распашной, угловая кухня, кухня в хрущёвке, цена шкафа-купе, наполнение шкафа-купе, фартук для кухни, цена гардеробной, подсветка в мебели, пенал в ванную, кухня с островом, мебель для студии, массив vs шпон, кухня под потолок, сканди-кухня, кровать-подиум. Conventions these follow (and any new post should too): **8 FAQ items** (the checklist's 4–8 range — the older posts' 9–10 predate the checklist), **5 contextual internal links** (checklist 3–5, not counting nav/crumbs/calculator/footer), **2 external sources** opening in a new tab with `rel="noopener"`, 1500+ words of body prose *excluding* the FAQ, title 50–60 and description 150–160 chars. Pricing posts compute every figure from `main.js` `CATEGORIES` with the calculator's own formula `max(dim×rate×mult, min×mult)` rounded to 100 ₽ — reproduce it in node before quoting a number. Facts must stay consistent with older posts (e.g. comfortable passage 700–800 mm in a walk-in wardrobe, 900–1200 mm around a kitchen island, upper cabinets 300–320 mm deep); read the sibling posts before writing.

Sources already verified via search (docs.cntd.ru unless noted): ГОСТ 13025.1-85 (1200017612, functional storage/kitchen-cabinet dimensions), ГОСТ 16371-2014 (1200121462), ГОСТ 19917-2014 (1200121720, sitting/lying furniture), ГОСТ 2977-82 (1200008207, sliced veneer), ГОСТ 13996-2019 (1200169000, ceramic tile), ГОСТ 30698-2014 (1200119828, tempered glass), ГОСТ 14254-2015 (1200136066, IP codes), ГОСТ Р 50571.7.701-2024 (1306610015, bathroom electrical zones — replaced the 2013 edition), ТР ТС 025/2012 and ТР ТС 004/2011 (official EEC pages on eec.eaeunion.org), СП 30.13330.2020 (minstroyrf.gov.ru/docs/117293/, drain slope 0.03 for 40–50 mm pipes), постановление Правительства РФ № 410 (consultant.ru, gas-equipment access and ventilation).

Two planned topics were swapped to avoid cannibalising existing posts: «Фасады МДФ: эмаль, плёнка, пластик» (already a section of the gloss-vs-matte post) became «Фартук для кухни», and «На чём нельзя экономить при заказе кухни» (already two sections of the kitchen-price post) became «Кухня под потолок». Check existing H2s for overlap before picking a topic.

Same pass: the base font token `--step-0` min was raised from 0.98rem to 1rem and `.article-body table` now uses `--step-0`, so article body text is ≥16px on mobile (checklist §12) — mono UI labels (ToC heading, table headers, buttons) stay smaller by design. The static `#calcPrice` placeholder on `/` and `/shkaf-kupe/` said «от 26 000 ₽» while the JS computes 27 000 ₽ for the 2 m default; fixed to match. Playwright pass after the last batch: 43 pages, zero overflow at 390/1024/1440, FAQ/ToC/back-to-top functional, no console errors, no broken links or missing anchors.
