...
**Homepage (`index.html`) is built** as the "мебель на заказ екатеринбург" hub page (structure informed by e-kupe.ru and kupeural.ru's category-page pattern, but the visual design and copy are original — no elements were copied). Sections: hero → category grid (шкафы-купе links to its real page, `/shkaf-kupe/`; the other 7 categories are still on-page anchors until they get real pages) → price calculator (client-side, ballpark estimates only — clearly labeled as approximate, no fabricated fixed SKU prices) → process steps → trust/differentiators → FAQ → lead form (client-side demo only, no backend — shows a success state, doesn't send anywhere yet) → footer.

## Category pages (started, ahead of the phased plan)

`/shkaf-kupe/index.html` is the first real category page (CLAUDE.md's Phase 1 category layer, pulled forward same as the blog). Structure informed by a 13-competitor structural survey (Wordstat/competitor research, see chat history — not re-derivable from files here): breadcrumb → hero (eyebrow/H1/lead/trust-row) → subcategory cards (встроенные/корпусные/угловые — descriptive only, no fake product grid) → calculator (same component as the homepage, defaults to шкаф-купе) → category-specific advantages → SEO text (links to the two relevant blog posts) → category FAQ → lead form (duplicated here rather than linking back to the homepage, since this is a landing page in its own right, not an article). Deliberately has no product grid with photos/fixed prices per model, unlike every competitor surveyed — PRODUCT.md bans fabricating a catalog that doesn't exist; the calculator is the honest substitute. New category pages: follow this template, add to `sitemap.xml`, and point the matching homepage cat-row link + all nav "category name" links at the new page instead of the homepage anchor.
